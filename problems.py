...
from qualification_round.problem2 import build_lookup_list, lookup_combo
...
def Metabolite_Annotation(lines):
    ...
    results = []
    for case in cases:
        print(f'Building lookup list for case {cases.index(case) + 1}/{len(cases)}')
        lookup_list = build_lookup_list(case['m'], case['a'])
        for signal in tqdm(case['s']):
            results.append(lookup_combo(lookup_list, signal))
    return results
